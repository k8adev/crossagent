...
Requirements: Node.js 22 or newer, tmux, and two panes each running a coding agent (Claude Code, Codex CLI).
...
Without step 2, the protocol is still available as the `pair` MCP prompt (`/crossagent:pair` in Claude Code). Details in [docs/skill.md](docs/skill.md).
...
- **brief** — the driver sends the peer the task and the context it needs.
- **discuss** — every plan and diff goes through the peer. The reply ends in one verdict line.
...
Each pane resolves its own tmux pane id, harness, and repo identity on startup, then self-declares them as tmux pane options so `list_peers` can find it. Codex CLI starts MCP servers lazily, so a Codex pane may not be discoverable by its declared identity until its first tool call. See [docs/discovery.md](docs/discovery.md) for the full resolution logic and fallbacks.
...
Using another coding agent? Adding a harness is a config entry, PRs welcome.
